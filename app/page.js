...
export default function Home() {
  ...
  return (
    ...
    // </div>
    <div
      className={`flex flex-col min-h-screen justify-between p-8 font-[family-name:var(--font-geist-sans)] ${
        contextTheme === "dark"
          ? "bg-gray-900 text-white"
          : "bg-white text-gray-800"
      }`}
    >
      ...
      <div className="grid grid-cols-4 gap-x-20 items-center sm:items-start">
        {sortedLocations.slice(0, 4).map((location, index) => {
          return (
            <div key={index}>
              <DownloadLink name={location.name} link={location.link} />
            </div>
      ...
      <div className="grid grid-cols-4 gap-x-20 items-center sm:items-start">
        {sortedLocations.slice(4, 8).map((location, index) => {
          return (
            <div key={index}>
              <DownloadLink name={location.name} link={location.link} />
            </div>
      ...
      <div className="grid grid-cols-3 gap-x-20 mb-[1rem]  items-center">
        {sortedLocations.slice(8, 11).map((location, index) => {
          return (
            <div key={index}>
              <DownloadLink name={location.name} link={location.link} />
      ...
      <div className="mt-[-1rem] col-span-full flex justify-center">
        <DownloadLink
          name="All Provinces/Territories"
          link="https://www.parklanesys.com/Downloads/9497/EXE/PKV12ALL.exe"
        />
